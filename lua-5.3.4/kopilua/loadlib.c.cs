...
namespace KopiLua
{
	...
	public partial class Lua
	{
		...
		** Set a path
		*/
		private static void setpath (lua_State L, CharPtr fieldname,
		                                   CharPtr envname,
		                                   CharPtr dft) {
		  CharPtr nver = lua_pushfstring(L, "%s%s", envname, LUA_VERSUFFIX);
		  CharPtr path = getenv(nver);  /* use versioned name */
		  if (path == null)  /* no environment variable? */
		    path = getenv(envname);  /* try unversioned name */
		  if (path == null || 0!=noenv(L))  /* no environment variable? */

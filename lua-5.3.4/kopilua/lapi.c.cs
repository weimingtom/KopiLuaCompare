	...
	public partial class Lua
	{
		...
		*/
		...
		public static void lua_xmove (lua_State from, lua_State to, int n) {
		  ...
		  for (i = 0; i < n; i++) {
			...
			lua_TValue.inc(ref to.top);  /* stack already checked by previous 'api_check' */
		  }
		  lua_unlock(to);
		}
		...
		*/
		...
		public static CharPtr lua_tolstring (lua_State L, int idx, out uint len) {
		  ...
		  //if (len != NULL) //FIXME: no if (len != null)
		    len = vslen(o);
		  return svalue(o);
		}
		...
		*/
		...
		private static int auxgetstr (lua_State L, TValue t, CharPtr k) {
		  TValue slot = null;
		  TString str = luaS_new(L, k);
//		  if (str.ToString().Equals("print"))
//		  {
//		  	Debug.WriteLine("auxgetstr got print");
//		  }
		  if (0!=luaV_fastget_luaH_getstr(L, t, str, ref slot)) {
		...
		public static int lua_geti (lua_State L, int idx, lua_Integer n) {
		  ...
		  lua_lock(L);
		  t = index2addr(L, idx);
		  if (0!=luaV_fastget_luaH_getint(L, t, n, ref slot)) {
		    setobj2s(L, L.top, slot);
		    api_incr_top(L);
		  }
		  else {
...
//}		
		...
		*/
		private static void auxsetstr (lua_State L, TValue t, CharPtr k) {
		  TValue slot = null;
		  TString str = luaS_new(L, k);
		  api_checknelems(L, 1);
		  if (0!=luaV_fastset_luaH_getstr(L, t, str, ref slot, L.top - 1))
		  	lua_TValue.dec(ref L.top);  /* pop value */
		  else {
		...
		public static void lua_seti (lua_State L, int idx, lua_Integer n) {
		  ...
		  api_checknelems(L, 1);
		  t = index2addr(L, idx);
		  if (0!=luaV_fastset_luaH_getint(L, t, n, ref slot, L.top - 1))
		  	lua_TValue.dec(ref L.top);  /* pop value */
  		  else {
		    setivalue(L.top, n);
		    api_incr_top(L);
		...
		*/
		...
		public static int lua_pcallk (lua_State L, int nargs, int nresults, int errfunc,
		  ...
		  if (k == null || L.nny > 0) {  /* no continuation or no yieldable? */
		    ...
		    L.errfunc = func;
		    setoah((byte)ci.callstatus, L.allowhook);  /* save value of 'allowhook' */

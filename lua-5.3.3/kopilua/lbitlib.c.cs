...
namespace KopiLua
{
	...
	public partial class Lua
	{
		...
		private static void pushunsigned(lua_State L, lua_Unsigned n)	{ lua_pushinteger(L, (lua_Integer)(n)); }
		private static lua_Unsigned checkunsigned(lua_State L, int i)	{ return ((lua_Unsigned)luaL_checkinteger(L,i)); }
		...
		*/
		...
		private static int b_shift (lua_State L, lua_Unsigned r, lua_Integer i) {
		  if (i < 0) {  /* shift right? */
		    i = -i;
            r = trim(r);
		    if (i >= LUA_NBITS) r = 0;
		...
		private static int b_rot (lua_State L, lua_Integer d) {
		  lua_Unsigned r = checkunsigned(L, 1);
		...
		*/
		...
		private static int b_replace (lua_State L) {
		  int w;
		  lua_Unsigned r = trim(checkunsigned(L, 1));
		  lua_Unsigned v = checkunsigned(L, 2);
		  int f = fieldargs(L, 3, out w);
		  lua_Unsigned m = mask(w);
		  r = (uint)((r & ~(m << f)) | ((v & m) << f)); //FIXME:changed, (uint)
		  pushunsigned(L, r);
		  return 1;
		}

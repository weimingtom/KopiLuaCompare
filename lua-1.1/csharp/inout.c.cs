...
{
 int c = fgetc (fp);
   if (0) fprintf(stdout, "=============>fileinput %c\n", c);
 return (c == EOF ? 0 : c);
}
...
{
   if (0) fprintf(stdout, "=============>lua_openfile %s\n", fn);
 lua_linenumber = 1;
 lua_setinput (fileinput);
 if (0) fprintf(stdout, "=============>lua_openfile 2 %s\n", fn);
 fp = fopen (fn, "r");
 if (0) fprintf(stdout, "=============>lua_openfile 3 %s\n", fn);
 if (fp == NULL) return 1;
 if (0) fprintf(stdout, "=============>lua_openfile 4 %s\n", fn);
 if (lua_addfile (fn)) return 1;
 if (0) fprintf(stdout, "=============>lua_openfile 5 %s\n", fn);
 return 0;
}

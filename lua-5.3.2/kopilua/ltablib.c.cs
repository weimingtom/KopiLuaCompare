...
namespace KopiLua
{
	...
	public partial class Lua
	{
		...
		*/
		...
		public static lua_Integer aux_getn(lua_State L, int n, int w)	{  checktab(L, n, (w) | TAB_L); return luaL_len(L, n); }


		private static int checkfield (lua_State L, CharPtr key, int n) {
		  lua_pushstring(L, key);
		  return (lua_rawget(L, -n) != LUA_TNIL)?1:0;
		}
		...
		*/
		private static void checktab (lua_State L, int arg, int what) {
		    ...
		    if (0!=lua_getmetatable(L, arg) &&  /* must have metatable */
		        ((0==(what & TAB_R)) || 0!=checkfield(L, "__index", ++n)) &&
		        ((0==(what & TAB_W)) || 0!=checkfield(L, "__newindex", ++n)) &&
		        ((0==(what & TAB_L)) || 0!=checkfield(L, "__len", ++n))) {
...
//#endif
		...
		*/
		...
		private static int tconcat (lua_State L) {
		  ...
		  lua_Integer i = luaL_optinteger(L, 3, 1);
  		  last = luaL_opt_integer(L, luaL_checkinteger, 4, last);
		  luaL_buffinit(L, b);
		  for (; i < last; i++) {
		    addfield(L, b, i);
		...
		*/
		...
		private static void set2 (lua_State L, uint i, uint j) {
		...
		*/
		...
		private static bool partition_f1(lua_State L, ref uint i)
		{
			++i; lua_geti(L, 1, (lua_Integer)i);
			return sort_comp(L, -1, -2)!=0;
		}
		private static bool partition_f2(lua_State L, ref uint j)
		{
			--j; lua_geti(L, 1, (lua_Integer)j);
			return sort_comp(L, -3, -1)!=0;
		}
		/*
		...
		** returns 'i'.
		*/
		private static uint partition (lua_State L, uint lo,
		                                             uint up) {
		...
		** "randomized" by 'rnd'
		*/
		private static uint choosePivot (uint lo, uint up,
		                                 uint rnd) {
		  uint r4 = (uint)(up - lo) / 4u;  /* range/4 */
		  uint p = rnd % (r4 * 2) + (lo + r4);
		...
		*/
		private static void auxsort (lua_State L, uint lo, uint up,
                                   uint rnd) {

...
namespace KopiLua
{
	public class Program
	{
		...
		private const string LUA_INITVARVERSION = LUA_INIT_VAR + Lua.LUA_VERSUFFIX;
		...
		*/
		...
		static void print_usage(Lua.CharPtr badoption) {
		  Lua.lua_writestringerror("%s: ", progname);
		  if (badoption[1] == 'e' || badoption[1] == 'l')
		    Lua.lua_writestringerror("'%s' needs argument\n", badoption);
		  else
		    Lua.lua_writestringerror("unrecognized option '%s'\n", badoption);
		  Lua.lua_writestringerror( //FIXME:???%s
		...
		*/
		static void l_message(Lua.CharPtr pname, Lua.CharPtr msg) {
			if (pname != null) Lua.lua_writestringerror("%s: ", pname);
  			Lua.lua_writestringerror("%s\n", msg);
		}
		...
		*/
		static int report(Lua.lua_State L, int status) {
		  if (status != Lua.LUA_OK) {
		    ...
		    Lua.lua_pop(L, 1);  /* remove message */
		  }
		  return status;
		}
		...
		*/
		static int msghandler (Lua.lua_State L) {
		  ...
		  if (msg == null) {  /* is error object not a string? */
		    if (0!=Lua.luaL_callmeta(L, 1, "__tostring") &&  /* does it have a metamethod */
		        Lua.lua_type(L, -1) == Lua.LUA_TSTRING)  /* that produces a string? */
		      return 1;  /* that is the message */
		    else
		      msg = Lua.lua_pushfstring(L, "(error object is a %s value)",
		                               Lua.luaL_typename(L, 1));
		  }
		  Lua.luaL_traceback(L, L, msg, 1);  /* append a standard traceback */
		...
		*/
		...
		static void print_version() {
		  Lua.lua_writestring(Lua.LUA_COPYRIGHT, (uint)Lua.strlen(Lua.LUA_COPYRIGHT)); //FIXME:changed, (uint)
		  Lua.lua_writeline();
		...
		*/
		static int pushline(Lua.lua_State L, int firstline) {
			...
			if ((firstline!=0) && (b[0] == '='))  /* for compatibility with 5.2, ... */
				...
				Lua.lua_pushlstring(L, b, (uint)l);
			lua_freeline(L, b);
			return 1;
		}
		...
		*/
		private static int addreturn (Lua.lua_State L) {
		  Lua.CharPtr line = Lua.lua_tostring(L, -1);  /* original line */
		  Lua.CharPtr retline = Lua.lua_pushfstring(L, "return %s;", line);
		  int status = Lua.luaL_loadbuffer(L, retline, (uint)Lua.strlen(retline), "=stdin");
		  if (status == Lua.LUA_OK) {
		    Lua.lua_remove(L, -2);  /* remove modified line */
		    if (line[0] != '\0')  /* non empty? */
		      lua_saveline(L, line);  /* keep history */
		  }
		  else
		    Lua.lua_pop(L, 2);  /* pop result from 'luaL_loadbuffer' and modified line */
		  return status;
		}
		...
		static void l_print (Lua.lua_State L) {
		  ...
		  if (n > 0) {  /* any result to be printed? */
		    ...
		    if (Lua.lua_pcall(L, n, 0, 0) != Lua.LUA_OK)
		      l_message(progname, Lua.lua_pushfstring(L, "error calling 'print' (%s)",
		...
		*/
		static int pushargs (Lua.lua_State L) {
		  int i, n;
		  if (Lua.lua_getglobal(L, "arg") != Lua.LUA_TTABLE)

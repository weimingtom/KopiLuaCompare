	...
	public partial class Lua
	{
		...
		*/
		...
		public static void luaL_traceback (lua_State L, lua_State L1,
		  ...
		  while (0!=lua_getstack(L1, level++, ar)) {
		...
		*/
		...
		private static object newbox (lua_State L, uint newsize) {
		  UBox box = (UBox)lua_newuserdata(L, typeof(UBox));
		  box.box = null;
		  box.bsize = 0;
		  if (0!=luaL_newmetatable(L, "LUABOX")) {  /* creating metatable? */
		...
		*/
		public static CharPtr luaL_prepbuffsize (luaL_Buffer B, uint sz) {
		  ...
		  if (B.size - B.n < sz) {  /* not enough space? */
		    ...
		    if (0!=buffonstack(B))
		      newbuff = (CharPtr)resizebox(L, -1, newsize);
		    else {  /* no buffer yet */
		      newbuff = (CharPtr)newbox(L, newsize);
		      memcpy(newbuff, B.b, B.n * 1/*sizeof(char)*/);  /* copy original content */
		    }
		    B.b = newbuff;
		    B.size = newsize;
		  }
		...
		*/
		...
		private static int skipBOM (LoadF lf) {
		  CharPtr p = "\xEF\xBB\xBF";  /* UTF-8 BOM mark */
		  int c;
		  lf.n = 0;
		  do {
		    c = getc(lf.f);
		...
		*/
        ...
        public static CharPtr luaL_tolstring (lua_State L, int idx, out uint len) { //FIXME: size_t * -> out uint
		  if (0!=luaL_callmeta(L, idx, "__tostring")) {  /* metafield? */
		    ...
		    switch (lua_type(L, idx)) {
		      ...
		      default: {
		        ...
		        CharPtr kind = (tt == LUA_TSTRING) ? lua_tostring(L, -1) :
		                                                 luaL_typename(L, idx);
		...
		public static void luaL_checkversion_ (lua_State L, lua_Number ver, uint sz) {
		  ...
		  if (v != lua_version(null))
		    ...
		    luaL_error(L, "version mismatch: app. needs %f, Lua core provides %f",
		                  (LUAI_UACNUMBER)ver, (LUAI_UACNUMBER)v[0]);
		}

...
namespace KopiLua
{
	...
	public partial class Lua
	{
		...
		private static LClosure ci_func(CallInfo ci) { return (clLvalue(ci.func)); }
		...
		*/
		...
		private static CharPtr findvararg (CallInfo ci, int n, ref StkId pos) {
		  ...
		  if (n >= cast_int(ci.u.l.base_ - ci.func) - nparams)
		...
		private static CharPtr getfuncname (lua_State L, CallInfo ci, ref CharPtr name) {
		  if (ci == null)  /* no 'ci'? */
		    return null;  /* no info */
		  else if (0!=(ci.callstatus & CIST_FIN)) {  /* is this a finalizer? */
		    name = "__gc";
		    return "metamethod";  /* report it as such */
		  }
		  /* calling function is a known Lua function? */
		  else if (0==(ci.callstatus & CIST_TAIL) && 0!=isLua(ci.previous))
		    return funcnamefromcode(L, ci.previous, ref name);
		...
		private static int auxgetinfo (lua_State L, CharPtr what, lua_Debug ar,
		  ...
		  for (; what[0] != 0; what = what.next()) {
			switch (what[0]) {
			  ...
			  case 'n': {
				ar.namewhat = getfuncname(L, ci, ref ar.name);
				if (ar.namewhat == null) {
		...
		private static CharPtr funcnamefromcode (lua_State L, CallInfo ci,
		  ...
		  switch (GET_OPCODE(i)) {
		    ...
		    case OpCode.OP_SELF: case OpCode.OP_GETTABUP: case OpCode.OP_GETTABLE:
		      tm = TMS.TM_INDEX;
		      break;
		    case OpCode.OP_SETTABUP: case OpCode.OP_SETTABLE:
		      tm = TMS.TM_NEWINDEX;
		      break;
		...
		*/
		...
		private static CharPtr varinfo (lua_State L, TValue o) {
		  CharPtr name = null;  /* to avoid warnings */
		  CallInfo ci = L.ci;
		  CharPtr kind = null;
		  if (isLua(ci)!=0) {
		...
		public static void/*l_noret*/ luaG_typeerror (lua_State L, TValue o, CharPtr op) {
		  CharPtr t = luaT_objtypename(L, o);
		...
		public static void/*l_noret*/ luaG_opinterror (lua_State L, TValue p1,
		                                               TValue p2, CharPtr msg) {
		  lua_Number temp = 0;
		...
		*/
		...
		public static void/*l_noret*/ luaG_ordererror (lua_State L, TValue p1, TValue p2) {
		  CharPtr t1 = luaT_objtypename(L, p1);
		  CharPtr t2 = luaT_objtypename(L, p2);
		  if (strcmp(t1, t2) == 0)
		...
		public static CharPtr luaG_addinfo (lua_State L, CharPtr msg, TString src,
                                                         int line) {
		  CharPtr buff = new CharPtr(new char[LUA_IDSIZE]);
	      if (src != null)
		...
		public static void/*l_noret*/ luaG_errormsg (lua_State L) {
		  if (L.errfunc != 0) {  /* is there an error handling function? */
			...
			StkId.inc(ref L.top);  /* assume EXTRA_STACK */
		...
		public static void/*l_noret*/ luaG_runerror(lua_State L, CharPtr fmt, params object[] argp) {
		  ...
		  if (0!=isLua(ci))  /* if Lua function, add source:line information */

...
namespace KopiLua
{
	...
	public class Program_luac
	{
		...
		{
		 if (message[0]=='-')
		  Lua.fprintf(Lua.stderr,"%s: unrecognized option '%s'\n",progname,message);
		 else
		  Lua.fprintf(Lua.stderr,"%s: %s\n",progname,message);
		...
		private static void PrintConstant(Lua.Proto f, int i)
		{
		 ...
		 switch (Lua.ttype(o))
		 {
		  ...
		  case Lua.LUA_TNUMFLT:
			{
			Lua.CharPtr buff = new Lua.CharPtr(new char[100]);
			Lua.sprintf(buff,Lua.LUA_NUMBER_FMT,Lua.fltvalue(o));
			Lua.printf("%s",buff);
			if (buff[Lua.strspn(buff,"-0123456789")]=='\0') Lua.printf(".0");
			break;
			}

	...
	public partial class Lua
	{
		...
		*/
		...
		private static int l_checkmode(CharPtr mode) {
			...
			if (!(strchr("rwa", mode[0]) != null)) { mode.inc(); return 0;}
			mode.inc();
			if (!(mode[0] != '+')) return 0;
			mode.inc(); /* skip if char is '+' */
			if (!(strspn(mode, L_MODEEXT) == strlen(mode))) return 0;
		...
		*/
		...
		public static int IOPREF_LEN = IO_PREFIX.Length; //(sizeof(IO_PREFIX)/sizeof(char) - 1)
		...
		*/
		...
		private static int io_open (lua_State L) {
		  ...
		  luaL_argcheck(L, 0!=l_checkmode(md), 2, "invalid mode");
		...
		*/
		...
		private class RN {
		  ...
		  public CharPtr buff = new CharPtr(new char[L_MAXLENNUM + 1]);  /* +1 for ending '\0' */
		};
		...
		*/
		private static int read_number (lua_State L, StreamProxy f) {
		  ...
		  int count = 0;
		  int hex = 0;
		  CharPtr decp = new CharPtr(new char[2]);
		  rn.f = f; rn.n = 0;
		  ...
		  if (0!=lua_stringtonumber(L, rn.buff))  /* is this a valid number? */
		    return 1;  /* ok */
		  else {  /* invalid format */
		...
		private static int read_line (lua_State L, StreamProxy f, int chop) {
		  ...
		  while (c != EOF && c != '\n') {  /* repeat until end of line */
		    CharPtr buff = luaL_prepbuffer(b);  /* preallocate buffer */
		    int i = 0;
		    l_lockfile(f);  /* no memory errors can happen inside the lock */
		    while (i < LUAL_BUFFERSIZE && (c = l_getc(f)) != EOF && c != '\n')
		      buff[i++] = (char)c;
		    l_unlockfile(f);
		    luaL_addsize(b, (uint)i);
		  }
		  ...
		  if (nargs == 0) {  /* no arguments? */
			...
			for (n = first; (nargs-- != 0) && (success!=0); n++) {
			  if (lua_type(L, n) == LUA_TNUMBER) {
				uint l = (uint)luaL_checkinteger(L, n);
		...
		private static int io_readline (lua_State L) {
		  ...
		  if (0!=lua_toboolean(L, -n))  /* read at least one value? */

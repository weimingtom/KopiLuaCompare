...
namespace KopiLua
{
	...
	public partial class Lua
	{
		...
		*/
		...
		private static void markobject(global_State g, GCObject t)	{ if (iswhite(t)) reallymarkobject(g, obj2gco(t)); }

		/*
		** mark an object that can be NULL (either because it is really optional,
		** or it was stripped as debug info, or inside an uncompleted structure)
		*/
		private static void markobjectN(global_State g, GCObject t)	{ if (t!=null) markobject(g,t); }
		...
		*/
		...
		private static lu_mem traversetable (global_State g, Table h) {
		  ...
		  return (uint)(GetUnmanagedSize(typeof(Table)) + GetUnmanagedSize(typeof(TValue)) * h.sizearray +
		                GetUnmanagedSize(typeof(Node)) * (uint)allocsizenode(h));
		}
		...
		*/
		static GCObjectRef sweeptolive (lua_State L, GCObjectRef p) {
		  GCObjectRef old = p;
		  do {
		    p = sweeplist(L, p, 1);
		  } while (p == old);
		...
		*/
		private static void callallpendingfinalizers (lua_State L) {
		  global_State g = G(L);
		...
		*/
		private static void entersweep (lua_State L) {
		  ...
		  g.sweepgc = sweeplist(L, new AllGCRef(g), 1);
		}
		...
		private static l_mem atomic (lua_State L) {
		  ...
		  remarkupvals(g);
		  propagateall(g);  /* propagate changes */
		  work = (int)g.GCmemtrav;  /* stop counting (do not recount 'grayagain') */
		  g.gray = grayagain;

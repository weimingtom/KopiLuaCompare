...
//#endif
		...
		*/
		...
		private const lua_Unsigned HIGHERBYTE = ((lua_Unsigned)MC << (NB * (SZINT - 1)));
		...
		*/
		...
		private static int getnum (ref CharPtr fmt, int df) {
		  if (0==digit(fmt[0]))  /* no number? */
		    ...
		    } while (0!=digit(fmt[0]) && a < (INT_MAX/10 - 10));
		    return a;
		  }
		}
		...
		*/
		private static KOption getoption (Header h, ref CharPtr fmt, ref int size) {
		  ...
		  switch (opt) {
		    ...
		    case 'c': size = getnum(ref fmt, 1); return KOption.Kchar;
		    case 'z': return KOption.Kzstr;
		    case 'x': size = 1; return KOption.Kpadding;
		    case 'X': return KOption.Kpaddalign;
		    case ' ': return KOption.Knop;
		    case '\0': return KOption.Keof;
		    case '<': h.islittle = 1; return KOption.Knop;
		    case '>': h.islittle = 0; return KOption.Knop;
		    case '!': h.maxalign = getnumlimit(h, ref fmt, MAXALIGN); return KOption.Knop;
		    default: {
		...
		*/
		private static KOption getdetails (Header h, uint totalsize,
		  ...
		  KOption opt;
		  do {
		    opt = getoption(h, ref fmt, ref psize);
		  } while (opt == KOption.Knop);  /* skip no-op options */
		  align = psize;  /* usually, alignment follows size */
		  if (opt == KOption.Kpaddalign) {  /* 'X' gets alignment from following option */
		    if (getoption(h, ref fmt, ref align) == KOption.Kchar || align == 0)
		...
		*/
		  ...
		  for (;;) {
		    ...
		    switch (opt) {
		      ...
		      case KOption.Kchar: {  /* fixed-size string */
		        ...
		        luaL_argcheck(L, len == (uint)size, arg, "wrong length");
		        luaL_addlstring(b, s, (uint)size);
		        break;
		      }
		...
		private static int str_unpack (lua_State L) {
		  ...
		  for (;;) {
		    ...
		    pos = (uint)(pos + ntoalign); //pos += ntoalign; //FIXME:

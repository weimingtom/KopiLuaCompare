	...
	public partial class Lua
	{
		...
		private static int luaB_tonumber (lua_State L) {
		  if (lua_isnoneornil(L, 2)) {  /* standard conversion? */
		    ...
		    lua_Integer base_ = luaL_checkinteger(L, 2);
		    ...
		    if (b_str2int(s, (int)base_, ref n) == s + l) {
		      lua_pushinteger(L, n);
		      return 1;
		    }  /* else not a number */
		  }  /* else not a number */
		...
		*/
		...
		private static int dofilecont (lua_State L, int d1, lua_KContext d2) {
		...
		** ignored).
		*/
		private static int finishpcall (lua_State L, int status, lua_KContext extra) {

...
namespace KopiLua
{
	public class Program
	{
		...
		*/
		static int pushargs (Lua.lua_State L) {
		  int i, n;
		  Lua.lua_getglobal(L, "arg");
		  if (!Lua.lua_istable(L, -1))
		    Lua.luaL_error(L, "'arg' is not a table");
		  n = (int)Lua.luaL_len(L, -1);
		  Lua.luaL_checkstack(L, n + 3, "too many arguments to script");
		  for (i = 1; i <= n; i++)
		    Lua.lua_rawgeti(L, -i, i);
		  Lua.lua_remove(L, -i);  /* remove table from the stack */
		  return n;
		}


		static int handle_script(Lua.lua_State L, Lua.StringPtr argv) {
			int status;
		...
		*/
		static int pmain(Lua.lua_State L) {
		  ...
		  if (script < argc &&  /* execute main script (if there is one) */
		    handle_script(L, new Lua.StringPtr(argv, script)) != Lua.LUA_OK)
		    return 0;

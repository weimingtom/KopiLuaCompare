	...
	public partial class Lua
	{
		...
		private static CharPtr b_str2int (CharPtr s, int base_, ref lua_Integer pn) {
		  ...
		  do {
		    ...
		    if (digit >= base_) return null;  /* invalid numeral */
		    n = (lua_Unsigned)(n * base_ + digit);
		    s.inc();
		  } while (isalnum((byte)s[0]));
		  s += strspn(s, SPACECHARS);  /* skip trailing spaces */
		  if (s[0] != '\0')  /* invalid trailing characters? */
		    return null;
		...
		*/
		private static int luaB_ipairs (lua_State L) {
		  lua_CFunction iter =
		     (0!=luaL_getmetafield(L, 1, "__len") ||
		      0!=luaL_getmetafield(L, 1, "__index"))
		  		? (lua_CFunction)ipairsaux : ipairsaux_raw;
		...
		*/
		...
		private static int dofilecont (lua_State L, int d1, lua_Ctx d2) {
		...
		** ignored).
		*/
		private static int finishpcall (lua_State L, int status, lua_Ctx extra) {

...
namespace KopiLua
{
	...
	public partial class Lua
	{
		...
		*/
		...
		public static lua_Unsigned luaL_checkunsigned (lua_State L, int arg) {
		  int isnum = 0; //FIXME: changed, =0
		  lua_Unsigned d = lua_tounsignedx(L, arg, ref isnum);
		  if (isnum==0)
		    interror(L, arg);
		  return d;
		}
		...
		public static lua_Unsigned luaL_optunsigned (lua_State L, int arg,
		                                                        lua_Unsigned def) {
		  return luaL_opt_unsigned(L, luaL_checkunsigned, arg, def);
		}
